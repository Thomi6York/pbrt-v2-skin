...
for subj = subjects
    ...
    values = [mel_std, - mel_std; hem_std, - hem_std;];
    %permute the values for mel and hem separately
    perms = unique(perms(values), 'rows');

    save(strcat(pigPath, perms.mat'), 'perms'); %save perms for ref

    %loop through the permutations
    count = 0;
    for i = perms
        count = count + 1;
        Out_Mel2 = Out_Mel + i(1);
        Out_Hem2 = Out_Hem + i(2);
        ...
        [~,betaInd] = min(abs(Out_Beta-Beta_sampling)); 
        [~,epthInd] = min(abs(Out_Epth-Epth_sampling));

        Out_Img2 = LUTs(betaInd,epthInd,melInd,hemInd,:); %lookup the new rgb values

        if debug
            disp('Displaying new texture');
            figure; imshow(Out_Img2);title(strcat('New texture with added ', num2str(i(1)), ' mel and ', num2str(i(2)), ' hem'));
        end

        %save the new maps
        save(strcat(pigPath, subj_id_string,'_PermID', count,'_.mat'),"Out_Epth2", "Out_Beta2","Out_Hem2","Out_Img2","Out_Mel2");

        %modulate the texture
        disp('Modulating texture via normalization with the 3rd percentile of the chromophores');

        fileNameHandle = strcat('PermID', count, '_');
        normTex= texNormalize(Out_Beta2,Out_Epth2,Out_Mel2,Out_Hem2, fileNameHandle, subj_id_string, texPath, pigPath, Mel_sampling, Hem_sampling, Beta_sampling, Epth_sampling, LUTs, face_mask, debug)
...
function normIm= texNormalize(Out_Beta2,Out_Epth2,Out_Mel2,Out_Hem2, fileNameHandle, subj_id_string, texPath, pigPath, Mel_sampling, Hem_sampling, Beta_sampling, Epth_sampling, LUTs, face_mask, debug)
    ...
    [~,betaInd] = min(abs(mean(Out_Beta2,'all')-Beta_sampling)); %set this to the mean 
    [~,epthInd] = min(abs(mean(Out_Epth2,'all')-Epth_sampling)); %set this to the mean
