...
%% note to self (Tom) -- i have edited this because it looks like it can't handle edges at the image boundary for some reason
%% second note -- this doesn't handle normalised images. Scale them to 0-255 before using this function.
%% 3rd note -- this is due to the uint8 conversion at the end of the function. If you really want to work in float just comment this out -- but make sure the preprocessing is done correctly.
    ...
    for x=1:size(map,1)
        for y=1:size(map,2)
            if im_mask(x,y)==1
                ...
                %check the boundary
                if x > 1 && im_mask(x-1,y)==0 %known left pixel, check boundary
                    B(counter)=im_target(x-1,y,i); %add it to B
                elseif x > 1 %unknown boundary
                    A(counter,map(x-1,y))=-1; %set its coefficient to -1
                end
                if x < size(map,1) && im_mask(x+1,y)==0 %known right pixel, check boundary
                    B(counter)=B(counter)+im_target(x+1,y,i); %add it to B
                elseif x < size(map,1) %unknown boundary
                    A(counter,map(x+1,y))=-1; %set its coefficient to -1
                end
                if y > 1 && im_mask(x,y-1)==0 %known bottom pixel, check boundary
                    B(counter)=B(counter)+im_target(x,y-1,i); %add it to B
                elseif y > 1 %unknown boundary
                    A(counter,map(x,y-1))=-1; %set its coefficient to -1
                end
                if y < size(map,2) && im_mask(x,y+1)==0 %known top pixel, check boundary
                    B(counter)=B(counter)+im_target(x,y+1,i); %add it to B
                elseif y < size(map,2) %unknown boundary
                    A(counter,map(x,y+1))=-1; %set its coefficient to -1
                end
