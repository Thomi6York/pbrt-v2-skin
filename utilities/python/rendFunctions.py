...
def sceneEditor(params,pathInfo,LightingCase,sample=1):
...
def readCacheFile(cache_file,subjects):
    with open(cache_file, "r") as cache_f:
        ...
        if subjNum not in subjects:
            print(f"Subject {subjNum} not in subjects list. Skipping with dummy params.")
...
def processFiles(pathInfo, cacheFile,batch_script,LightingCase,subjects,sample=1,overwriteALL=False,skipALL = False):
    ...
    with open(scene_file, "w") as scene_f:
        ...
        scene = sceneEditor(params,renderPathInfo, LightingCase, sample=sample)
        scene_f.write(scene.replace("\\", "\\\\"))
        if permID == "" or permID == '':
    ...
    if (not os.path.exists(renderPath)) | overwriteALL == True:
        ...
        G = input("Press Y to overwrite or any other key to skip, or YY for all, or NN to skip all:")
        if G == "Y" or G == "YY":
            scene_command = f'".\\bin\\pbrt.exe" "{scene_file}"\n'
            #f.write(scene_command)
            if G == "YY":
    ...
    return  overwriteALL, skipALL, scene_command