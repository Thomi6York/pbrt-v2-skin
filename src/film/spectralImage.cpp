...
void SpectralImageFilm::WriteImage(float splatScale) {
    std::cout << "got to WriteImage() for Spectral type Film\n";
    ...
    for (int y = 0; y < yPixelCount; ++y) {
        for (int x = 0; x < xPixelCount; ++x) {
            ...
            for (int row = 0; row < nCMRows; row++)
            {
                ...
                finalCMultiplied[nCMRows*offset2 + row] = tempSum;
                offset2++; //increment offset
            } // this should give an array of nCMRows * nPix size
    ...
    std::cout << "outputing: " << xPixelCount << " " << yPixelCount << " " << nCMRows << "\n";
    //for (int i = 0; i < nCMRows; i++)
    ...
    string newFileNameSpectral = filename.substr(0, lastPos) + "_spectral.exr";
    ::WriteImage(newFileNameSpectral, finalCMultiplied, NULL, xPixelCount, yPixelCount,
                xResolution, yResolution, xPixelStart, yPixelStart, nCMRows);
    std::cout << "nChannels: " << nCMRows << "\n";
    std::cout << "outputing: " << newFileNameSpectral << "\n";
   ...
   // Write Binary image -- ive made my own loop here to write the data in a more sensible way for reading in matlab
    // for (int i = 0; i < nCMRows; i++) {
    //     for (int y = 0; y < yPixelCount; y++) {
    //         for (int x = 0; x < xPixelCount; x++) {
    //             double r = (double)finalCMultiplied[nCMRows * (y * xPixelCount + x) + i];
    //             fwrite((void*)(&r), sizeof(r), 1, spectralImageBin);
    //         }
    //     }
    // }

    //write this as a table where there is an x pixel, y pixel, the spectral band, and the value of C at that pixel
    // for (int i = 0; i < nCMRows; i++)
    // {
    //     for (int y = 0; y < yPixelCount; y++)
    //     {
    ...
    //             double r = (double)finalCMultiplied[nCMRows * (y * xPixelCount + x) + i];
    //             if (y == 0  && x == 0){
    //             printf("Writing x: %d, y: %d, i: %d, r: %f\n", x, y, i, r);
    //             }
    //             fwrite((void*)(&x), sizeof(x), 1, spectralImageBin);
    //             fwrite((void*)(&y), sizeof(y), 1, spectralImageBin);
    //             fwrite((void*)(&i), sizeof(i), 1, spectralImageBin);
    //             fwrite((void*)(&r), sizeof(r), 1, spectralImageBin);
    //         }
    //     }
    // }
    ...
    // lets create a new text file to add the table to
    // string newFileNameTable = filename.substr(0, lastPos) + "_table.txt";
    // std::ofstream tableFile;
    // tableFile.open(newFileNameTable.c_str());
    // tableFile << "xPixel\tyPixel\tSpectralBand\tC\n";
    // for (int i = 0; i < nCMRows; i++)
    // {
    //     for (int y = 0; y < yPixelCount; y++)
    //     {
    ...
    std::cout << "outputing: " << newFileName << "\n";
    

    //Write Binary depth map file -- we can also check the binary file to match the exr file for structure 
    for (int j = 0; j < nPix; j++)
    {
    ...
    fclose(depthMapBin);
    std::cout << "outputing: " << newFileNameDM << "\n";
